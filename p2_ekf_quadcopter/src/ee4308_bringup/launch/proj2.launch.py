...
def generate_launch_description():
    ...
    turtle_x_arg = DeclareLaunchArgument('turtle_x', default_value='5.0')
    ld.add_action(turtle_x_arg)

    turtle_y = LaunchConfiguration('turtle_y')
    turtle_y_arg = DeclareLaunchArgument('turtle_y', default_value='3.0')
    ...
    turtle_description_config = xacro.process_file(
        os.path.join(pkg_ee4308_bringup, 'urdf', 'turtle_proj2.urdf.xacro'))
    ...
    node_turtle_static_tf = Node(
        ...
        executable='static_transform_publisher',
        arguments=['0', '0', '0', '0', '0', '0', 'map', f'{turtle_ns}/odom'],
        output='screen'
    )
    ...
    node_spawn_turtle = Node(
        ...
        output='screen',
        on_exit=[node_rviz, node_turtle_map_server, node_turtle_controller, node_turtle_planner, node_turtle_behavior, node_drone_estimator, node_drone_planner, node_drone_controller, node_drone_behavior]
    )
