#include <ee4308_gazebo_plugins/gazebo_ros_baro.hpp>
...
namespace gazebo
{
  ...
  void GazeboRosBaro::Load(physics::ModelPtr _model, sdf::ElementPtr _sdf)
  {
    ...
    if (!link)
    {
      RCLCPP_FATAL(node_->get_logger(), "GazeboRosBaro plugin error: bodyName: %s does not exist\n", link_name_.c_str());
      return;
    }
    ...
    if (_sdf->HasElement("topicName"))
      topic_altitude_ = _sdf->GetElement("topicName")->Get<std::string>();
    ...
    msg_altitude_.header.frame_id = frame_id_;
    ...
    if (!topic_altitude_.empty())
      pub_altitude_ = node_->create_publisher<geometry_msgs::msg::PointStamped>(topic_altitude_, 10);
  ...
  void GazeboRosBaro::Update()
  {
    ...
    if (pub_altitude_)
    {
      msg_altitude_.header.stamp = rclcpp::Time(sim_time.sec, sim_time.nsec);
      msg_altitude_.point.z = height + elevation_;
      pub_altitude_->publish(msg_altitude_);
    }

    // if (altitude_publisher_)
    // {
